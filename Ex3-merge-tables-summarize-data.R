...
#the play table contains information available for individual plays including time left on the clock, how many points the offense has, the down, the distance, etc. We can join that table to the pass table using the merge function. The pass table contains pass play specific information like who the target was, how many yards were gained, and who the passer was. I have two lines of code here to illustrate two ways to make this join. In the first call I'm going to keep all of the rows from playTbl (the x table) even if there is no match in the pass table. in the 2nd call I'm just keeping the rows from playTbl if there is a match in passTbl. The first call would be useful if you also intended on adding in rushing plays for instance. Be sure to inspect the resulting tables to see how they differ. the by.x argument tells it which field to join on. i'm saying that both tables contain a pid field and that's how R will know which rows should be joined.

playTblAll <- merge(playTbl,passTbl,by.x="pid",by.y="pid",all.x=T)

playTbl <- merge(playTbl,passTbl,by.x="pid",by.y="pid")

#now let's throw out some columns we're not going to use from the game table. we'll keep just the columns named by the list in the brackets below. ou is over/under. gid is a unique game id, wk is the week of the season.
...
#now we're going to just keep rows from the table where wk is less than 18. we're throwing out the playoffs when we do this.

playTbl <- playTbl[playTbl$wk<18,]

#there's a pts column in the table but we need something a little more exact because we're going to calculate fantasy points. the way the table is currently set up some pass plays yield negative points for a pick six. so the first thing we'll do is just create a column where td is equal to zero on every row.
...
#now we can merge that newly created table (sumTbl) with the game table which will give those summed up fantasy points some context because we'll also have over/under and day of the week for them.
...
#maybe now you want to know how many fantasy points are scored on average per team, based on the day of the week - do Thursdays really yield fewer fantasy points?. this line uses dplyr again and uses the same number of pipes we used above. the difference is that in this case we're grouping the data by day. to show you how customizable the calculations are I've also added a max calculation. almost any number you want to calculate could be added to this. You may note after you've run this code that the data contains a few typos in the day field. one thing you always have to be doing with any analysis is cleaning bad data.
...
#maybe now we want to figure out how many fantasy points a play generated compared to the average for the down/distance. we can take the actual fantasy points, subtract out the expected, and then what we have left over is fantasy points over expectation. this is sort of similar to the numbers in the fantasy efficiency app although that app is position specific and here we've just lumped every position together.
